...
def main():
    ...
    if uploaded_file is not None:
        ...
        file_path_input = st.text_area("Enter file path here:", height = 10, max_chars = 50)
        filepath = ''

        # I am writing the logic this way since this is how my local directory is set up
        if 'gl' in file_path_input:
            filepath = f'./data/archive/Testing/glioma/{file_path_input}'
        elif 'me' in file_path_input:
            filepath = f'./data/archive/Testing/meningioma/{file_path_input}'
        elif 'pi' in file_path_input:
            filepath = f'./data/archive/Testing/pituitary/{file_path_input}'
        elif 'no' in file_path_input:
            filepath = f'./data/archive/Testing/notumor/{file_path_input}'
        else:
            st.warning("Image naming convention is incorrect. Please try again.")
        #######################

        # Make predictions when the "Predict" button is clicked
        if st.button("Predict"):
            # Call your model_prediction function
            prediction, tumor_name, actual_tumor, class_probs = model_prediction(filepath)
